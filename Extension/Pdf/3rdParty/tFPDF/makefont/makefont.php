...
function MakeFont($fontfile, $enc='cp1252', $embed=true)
{
	...
	// TODO: Create [Patch] Directory
	$dirname = dirname($fontfile);
	...
	if($embed)
	{
		...
		{
			$file = $basename.'.z';
			SaveToFile( $dirname.DIRECTORY_SEPARATOR.$file, gzcompress($info['Data']), 'b');
			$info['File'] = $file;
	...
	MakeDefinitionFile( $dirname.DIRECTORY_SEPARATOR.$basename.'.php', $type, $enc, $embed, $map, $info);
